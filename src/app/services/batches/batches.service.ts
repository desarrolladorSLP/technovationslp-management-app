...
export class BatchesService {
  ...
  delete(id: string) {
    return this.httpClient.delete<Batch>(this.urlEndpoint + "/" + id).pipe(
      catchError(error => {
        console.log(error);
        return throwError(
          new TeckersError(error.error.error, error.error.message)
        );
      })
    );
  }
